...
public interface MsgDao extends JpaRepository<Msg,Integer> {
    @Query("select m from Msg m where receiverId=:receiverId and msgId=0 order by m.createTime desc")
    List<Msg> getRootMsgByReceiverId(@Param("receiverId") int receiverId);

    @Query("select m from Msg m where senderId=:senderId and msgId=0 order by m.createTime desc")
    List<Msg> getRootMsgBySenderId(@Param("senderId") int senderId);
    ...
    //通过msgId获取消息
    @Query("select m from Msg m where msgId=:msgId")
    Msg getMessageByMsgId(@Param("msgId") int msgId);

    //通过id获取消息
    @Query("select m from Msg m where id=:id")
    Msg getMessageById(@Param("id") int id);
}
